...
## 更新摘要
**所做更改**
- 移除了 Tauri API 相关内容的描述，因为该模块已被标记为废弃
- 更新了架构总览，反映 Electron Bridge 作为单一通信方式
- 强调了 Electron IPC 通信的统一性
- 更新了依赖关系分析，移除了 Tauri 相关依赖
...
**重要更新**：CoSurf 已完全迁移到 Electron IPC 通信，Tauri 支持已被移除。所有 API 现在都通过 Electron Bridge 进行通信。

## 项目结构
前端位于 src-web/src，核心 API 层由以下模块组成：
- **Electron Bridge 层**：封装 window.electronAPI 的 invoke/on/send/once/removeAllListeners，提供与 Electron 主进程的统一通信接口。
- **API 适配层**：对主进程命令进行分组封装，统一位置参数调用，内置 JSON 解析与错误处理。
- **事件适配层**：统一事件监听与一次性监听，提供事件常量与 Electron 环境下的事件处理。
...
- [api.ts:13-435](file://src-web/src/lib/api.ts#L13-L435)
...
**重要更新**：架构已简化为单一 Electron IPC 通信路径，移除了 Tauri 相关组件。
...
- **tauri.ts**
  ...
  - **重要说明**：此模块已标记为废弃，不推荐使用任何 Tauri 相关功能。
- **设计意图**
  - 明确迁移方向，避免混用两种桥接方案。
...
## 结论
CoSurf 前端 API 以 Electron Bridge 为核心，通过 API 适配层、事件适配层与事件管理器形成清晰的通信协议与数据流。浏览器自动化引擎与工具函数进一步增强了前端对页面与外部系统的控制能力。**重要更新**：架构已完全迁移到 Electron IPC，Tauri 支持已被移除。遵循本文档的接口规范与最佳实践，可确保前后端通信稳定、可维护且具备良好的用户体验。
...
### 迁移指南
由于 Tauri 支持已被移除，开发者需要：
1. 将所有 `@tauri-apps/api` 相关代码替换为 Electron Bridge API
2. 使用 `electronBridge.invoke()` 替代 `invoke()`
3. 使用 `electronBridge.on()` 替代 `listen()`
4. 使用 `electronBridge.send()` 替代 `emit()`
5. 移除对 `tauri.ts` 的依赖